...
def distance(FOCAL,typeVehicle,pred_length):
    if typeVehicle == 'person':
        R= HUMAN
    elif typeVehicle == 'motorcycle':
        R= BIKE
    elif typeVehicle == 'truck':
        R= TRUCK
    elif typeVehicle == 'car':
        R= CAR
    else: R= BUS
    r = pred_length*0.26
    d = FOCAL*(R/r)
...
def plot_one_box(x, img, color=None, label=None, line_thickness=3):
    ...
    if label:
        ...
        cv2.putText(img, str(round(d/1000,2)), (c1[0], c1[1] - 12), 0, tl / 3, [225, 255, 255], thickness=tf, lineType=cv2.LINE_AA)
        height=0
